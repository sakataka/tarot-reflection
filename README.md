# Tarot Reflection

タロットカードの象徴を使って相談内容を整理する、ローカル用の遊び・内省支援Webアプリです。カード抽選、正位置 / 逆位置、スプレッド割り当てはアプリ側で機械的に決め、AIは確定済み結果の解釈だけに使います。

## 起動

依存関係はBunで入れます。

```sh
bun install
```

通常はこちらを使います。画面表示とCodex App Server連携APIが同じ `4192` 番で動きます。

```sh
bun run dev
```

起動後、次を開きます。

http://127.0.0.1:4192/

カードを開くと、このサーバーから `codex app-server` を子プロセスとして起動し、読み解きを取得します。モデルは `gpt-6-sol`、推論強度は `medium` に固定しています。

ポートを変える場合:

```sh
PORT=4193 bun run server
```

フロントエンドだけをViteで確認する場合:

```sh
bun run dev:frontend
```

この場合、カードの読み解きを使うには別ターミナルで `bun run server` も起動してください。

## Codex連携

- フロントエンドにAPIキーは置きません。
- 事前にCodex CLIへログインしておく必要があります。
- Codex App Serverが使えない場合でも、画面のプロンプトをコピーして手動で貼り付けられます。

## 検証

```sh
bun test
bun run build
```

## UIと画像

夜の占い部屋を思わせる藍黒と古金のUIに、月明かり・藍色・金色をそろえたカード画像を合わせています。カードを混ぜると卓の上に弧状に広がり、選んだカードは結果画面で一枚ずつ表に返ります。効果音は Web Audio で合成しており、ヘッダーから切り替えられます（初期状態はオフ）。大アルカナ22点、小アルカナのスート別画像4点、裏面、導入の風景の全28点を `public/cards/` に保存しています。小アルカナ56枚はスートの画像にランクを重ねて区別します。

制作方針と再制作用のプロンプトは [画像の制作方針](docs/image-direction.md) を参照してください。
